...
# -*- coding: utf-8 -*-
"""
Functions for processing & analyzing UCSB_Smith data
...
def create_time_stamps(dic, stimulus, neuron_list):
    """Creates a COMBINED (across trials) time stamp for selected neurons.    
    """
...
def limited_xcorr_neuron(raster1, neuron1, raster2, neuron2, stimulus, limit_len, trial_num = None):
    ...
    # compare to similar computation after removing the first 53 and last 53 
    #list1 = [0]*53 + spikes1[53:n_bins-53] + [0]*53
    #list2 = [0]*53 + spikes2[53:n_bins-53] + [0]*53

    #x_corr_var = np.correlate(list1, list2, 'full')
    
    #normalize 
    #norm_factor_var = np.sqrt(np.dot(list1, list1) * np.dot(list2, list2))
    #corr_norm_var = [x/norm_factor_var for x in x_corr_var]
    
    #score_var = sum(corr_norm_var[n_bins-limit_len: n_bins+limit_len])
    
    #return max(score, score_var)

    return score
def limited_xcorr(raster, stimulus, neurons_idx, limit_len, trial_num = None):
    ...
    for (i,j) in combinations(range(n_neurons),2):
        score = limited_xcorr_neuron(raster, neurons_idx[i], raster, neurons_idx[j], stimulus, limit_len, trial_num = None)
...
def limited_xcorr_V1_AL(raster_1, neurons_idx_1, raster_2, neurons_idx_2, stimulus, limit_len):
    ...
    for (index, x) in np.ndenumerate(V1_AL_similarity):
        ...
        V1_AL_similarity[neuron_1, neuron_2] = limited_xcorr_neuron(raster_1, neurons_idx_1[neuron_1], raster_2, neurons_idx_2[neuron_2],'stimulus_1', 50)

    similarity_1, distance_1 = limited_xcorr(raster_1, 'stimulus_1', neurons_idx_1, 50)
    similarity_2, distance_2 = limited_xcorr(raster_2, 'stimulus_1', neurons_idx_2, 50)
